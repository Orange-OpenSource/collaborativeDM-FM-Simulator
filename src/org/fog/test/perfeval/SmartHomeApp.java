...
public class SmartHomeApp {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String appId = "SmartHome"; // identifier of the application
          ...
          /* FaultInjector fault=new FaultInjector("HighVariance",600500, sensors.get(0).getId(), FailureType.HIGHVARIANCE);
            ...
            Random r = new Random();
            int legacy=r.nextInt((1200000 - 900000) + 1) + 900000;

            double []dates= exp.generateFailureDates(6); //generate failures according to the uniform distribution.
            for (int j = 0; j < numberOfArea; j++) {


                for (int i = 0; i < 2; i++) {
                    FaultInjector fault1 = new FaultInjector("HighVariance", (int) dates[i], sensors.get(0).getId(), FailureType.HIGHVARIANCE);
                    failures.add(fault1);
                    FaultRecovery recovery1 = new FaultRecovery("recovery", (int) dates[i] + 2714, sensors.get(j*3+0).getId());
                    recoveryActions.add(recovery1);

                    FaultInjector fault2 = new FaultInjector("HighVariance", (int) dates[i], sensors.get(1).getId(), FailureType.HIGHVARIANCE);
                    failures.add(fault2);
                    FaultRecovery recovery2 = new FaultRecovery("recovery", (int) dates[i] + 3713, sensors.get(j*3+1).getId());
                    recoveryActions.add(recovery2);

                    FaultInjector fault3 = new FaultInjector("HighVariance", (int) dates[i], sensors.get(2).getId(), FailureType.HIGHVARIANCE);
                    failures.add(fault3);
                    FaultRecovery recovery3 = new FaultRecovery("recovery", (int) dates[i] + 3803, sensors.get(j*3+2).getId());
                    recoveryActions.add(recovery3);
                }
                for (int i = 2; i < 4; i++) {
                    FaultInjector fault1 = new FaultInjector("StuckAt", (int) dates[i], sensors.get(0).getId(), FailureType.STUCKAT);
                    failures.add(fault1);
                    FaultRecovery recovery1 = new FaultRecovery("recovery", (int) dates[i] +2714, sensors.get(j*3+0).getId());
                    recoveryActions.add(recovery1);

                    FaultInjector fault2 = new FaultInjector("StuckAt", (int) dates[i], sensors.get(1).getId(), FailureType.STUCKAT);
                    failures.add(fault2);
                    FaultRecovery recovery2 = new FaultRecovery("recovery", (int) dates[i] + 3713, sensors.get(j*3+1).getId());
                    recoveryActions.add(recovery2);

                    FaultInjector fault3 = new FaultInjector("StuckAt", (int) dates[i], sensors.get(2).getId(), FailureType.STUCKAT);
                    failures.add(fault3);
                    FaultRecovery recovery3 = new FaultRecovery("recovery", (int) dates[i] + 3803, sensors.get(j*3+2).getId());
                    recoveryActions.add(recovery3);
                }
             ...
             moduleMapping.addModuleToDevice("smoke_detector", "cloud");
             moduleMapping.addModuleToDevice("leak_detector", "cloud");
    ...
    private static void createFogDevices(int userId, String appId) {
        ...
        {
            addArea(String.valueOf(i),userId,appId,cloud.getId());
    ...
    private static void addArea(String id, int userId, String appId, int parentId){
        FogDevice hub = createFogDevice("smartThingsHub"+id, 2800, 4000, 10000, 10000, 0.0, 107.339, 83.4333);
        hub.setParentId(parentId);
        hub.setUplinkLatency(100);
        fogDevices.add(hub);
        //sensor declaration
        Sensor sensor = new MotionSensor("MotionSensor"+id, "MOTION", userId, appId, new DeterministicDistribution(5)); // inter-transmission time of camera (sensor) follows a deterministic distribution
        sensor.setGatewayDeviceId(hub.getId());
        sensor.setLatency(1.0);
        sensors.add(sensor);
        sensor= new SmokeSensor("SmokeSensor"+id, "SMOKE", userId, appId, new DeterministicDistribution(5)); // inter-transmission time of camera (sensor) follows a deterministic distribution
        sensor.setGatewayDeviceId(hub.getId());
        sensor.setLatency(1.0);
        sensors.add(sensor);
        sensor= new LeakSensor("LeakSensor"+id, "LEAK", userId, appId, new DeterministicDistribution(5)); // inter-transmission time of camera (sensor) follows a deterministic distribution
        sensor.setGatewayDeviceId(hub.getId());
        sensor.setLatency(1.0);
        sensors.add(sensor);
        //actuator declaration
        Actuator actuator = new MyActuator("Alarm"+id, userId, appId, "ALARM_CONTROL");
        ...
        actuator = new MyActuator("LightBulb"+id, userId, appId, "LIGHT_CONTROL");
        actuator.setGatewayDeviceId(hub.getId());
        actuator.setLatency(1.0);
        actuators.add(actuator);
        Actuator waterValve=new MyActuator("waterValve"+id, userId, appId, "VALVE_CONTROL");
    ...
    private static Application createApplication(String appId, int userId){
        ...
        application.addAppModule("smoke_detector", 10);
         ...
         * Connecting the application modules (vertices) in the application model (directed graph) with edges
         */
        application.addAppEdge("SMOKE", "smoke_detector", 2000, 2000, "SMOKE", Tuple.UP, AppEdge.SENSOR); // adding edge from CAMERA (sensor) to Motion Detector module carrying tuples of type CAMERA
        application.addAppEdge("smoke_detector", "ALARM_CONTROL", 14, 100, "ALARM_PARAMS", Tuple.DOWN, AppEdge.ACTUATOR); // adding edge from Object Tracker to PTZ CONTROL (actuator) carrying tuples of type PTZ_PARAMS

        /*
         * Defining the input-output relationships (represented by selectivity) of the application modules.
         */
        application.addTupleMapping("smoke_detector", "SMOKE", "ALARM_PARAMS", new FractionalSelectivity(1.0)); // 1.0 tuples of type MOTION_VIDEO_STREAM are emitted by Motion Detector module per incoming tuple of type CAMERA
        ...
        final AppLoop loop2 = new AppLoop(new ArrayList<String>(){{add("LEAK");add("leak_detector"); add("VALVE_CONTROL");}});
        final AppLoop loop3 = new AppLoop(new ArrayList<String>(){{add("SMOKE");add("smoke_detector"); add("ALARM_CONTROL");}});
