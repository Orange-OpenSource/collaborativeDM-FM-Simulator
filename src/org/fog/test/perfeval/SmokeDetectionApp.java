 ...
 *
 */
public class SmokeDetectionApp {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String appId = "smokeDetection"; // identifier of the application
           ...
           /* FaultInjector fault=new FaultInjector("HighVariance",60, sensors.get(0).getId(), FailureType.HIGHVARIANCE);
            ...
            failures.add(fault);
            FaultRecovery recovery= new FaultRecovery("recovery",600500+100, sensors.get(0).getId());
            recoveryActions.add(recovery);
            fault=new FaultInjector("stuckAt",600500+100+180,sensors.get(0).getId(),FailureType.STUCKAT);
            failures.add(fault);
            recovery= new FaultRecovery("recovery",600500+100+300, sensors.get(0).getId());
            recoveryActions.add(recovery);
            fault=new FaultInjector("stuckAt",600500+100+600,sensors.get(0).getId(),FailureType.STUCKAT);
            failures.add(fault);
            recovery= new FaultRecovery("recovery",600500+100+700, sensors.get(0).getId());
            recoveryActions.add(recovery);*/
            /**test protocol exp**/
           ExperimentationManager exp=new ExperimentationManager();
            ...
            for(int i=0;i<3;i++) {
                ...
                failures.add(fault);
                FaultRecovery recovery = new FaultRecovery("recovery", (int) dates[i] + 640 , sensors.get(0).getId());
                recoveryActions.add(recovery);
            }
            for(int i=3;i<7;i++) {
            ...
            moduleMapping.addModuleToDevice("smoke_detector", "cloud");  // fixing 1 instance of the Motion Detector module to each Smart Camera
    ...
    private static void createFogDevices(int userId, String appId) {
        ...
        fogDevices.add(hub);
        Sensor sensor = new MySensor("SmokeSensor", "SMOKE", userId, appId, new DeterministicDistribution(5)); // inter-transmission time of camera (sensor) follows a deterministic distribution
        sensors.add(sensor);
        Actuator actuator = new MyActuator("Alarm", userId, appId, "ALARM_CONTROL");
    ...
    private static Application createApplication(String appId, int userId){
         ...
         */
      final AppLoop loop1 = new AppLoop(new ArrayList<String>(){{add("SMOKE");add("smoke_detector"); add("ALARM_CONTROL");}});
